    ...
    * ExifToolAlpha - alpha-quality code which extends the ExifToolHelper to add functionality that is niche, brittle, or not well tested
    ...
    def execute_json_wrapper(self, filenames, params=None, retry_on_error=True):
        ...
        if result:
            try:
                ...
                if retry_on_error:
                    result = self.execute_json_wrapper(
    ...
    def _check_result_filelist(file_paths, result):
        ...
        if len(result) != len(file_paths):
            raise IOError(
                f"exiftool returned {len(result)} results, but expected was {len(file_paths)}"
            )
        ...
        for i in range(len(file_paths)):
            ...
            if Path(returned_source_file) != Path(requested_file):
                raise IOError(
                    f"exiftool returned data for file {returned_source_file}, but expected was {requested_file}"
                )
