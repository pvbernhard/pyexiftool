...
class ExifTool(object):
    ...
    def __init__(
        ...
        self._ver: Optional[str] = (
            None  # this is set to be the exiftool -v -ver when running
        )

        self._last_stdout: Optional[str] = None  # previous output
        self._last_stderr: Optional[str] = None  # previous stderr
        self._last_status: Optional[int] = (
            None  # previous exit status from exiftool (look up EXIT STATUS in exiftool documentation for more information)
        )
        ...
        self._executable: Union[str, Path] = (
            constants.DEFAULT_EXECUTABLE
        )  # executable absolute path (default set to just the executable name, so it can't be None)
        self._config_file: Optional[str] = (
            None  # config file that can only be set when exiftool is not running
        )
        self._common_args: Optional[List[str]] = None
        self._logger = None
        self._encoding: Optional[str] = None
        self._json_loads: Callable = (
            json.loads
        )  # variable points to the actual callable method
    ...
    def encoding(self, new_encoding: Optional[str]) -> None:
        ...
        self._encoding = new_encoding or (
            locale.getpreferredencoding(do_setlocale=False) or ENCODING_UTF8
        )
            ...
            if self._process.poll() is not None:
                ...
                if self._logger:
                    self._logger.warning(
                        "Property 'running': ExifTool process was previously running but died"
                    )
    ...
    def _set_logger(self, new_logger) -> None:
        ...
        if not check:
            raise TypeError(
                "logger needs to implement methods (info,warning,error,critical,exception)"
            )
    ...
    logger = property(
        fset=_set_logger,
        doc="""Write-only property to set the class of logging.Logger""",
    )
    """
    ...
    def run(self) -> None:
        ...
        try:
            ...
            raise ExifToolVersionError(
                f"Error retrieving Exiftool info.  Is your Exiftool version ('exiftool -ver') >= required version ('{constants.EXIFTOOL_MINIMUM_VERSION}')?"
            )
    ...
    def terminate(self, timeout: int = 30, _del: bool = False) -> None:
        ...
        if _del and constants.PLATFORM_WINDOWS:
            ...
            try:
                ...
                try:
                    self._process.communicate(
                        input=b"-stay_open\nFalse\n", timeout=timeout
                    )  # this is a constant sequence specified by PH's exiftool
                except RuntimeError:
                    # Python 3.12 throws a runtime error -- see https://github.com/python/cpython/pull/104826
                    # RuntimeError: can't create new thread at interpreter shutdown
                    pass
        ...
        if self._logger:
            self._logger.info("Method 'terminate': Exiftool terminated successfully.")
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def execute(
        self, *params: Union[str, bytes], raw_bytes: bool = False
    ) -> Union[str, bytes]:
        ...
        cmd_params.extend(
            (
                b"-echo4",
                f"{SEQ_ERR_STATUS_DELIM}{seq_err_status}{SEQ_ERR_STATUS_DELIM}{seq_err_post}".encode(
                    self._encoding
                ),
        ...
        if self._logger:
            self._logger.info(
                "Method 'execute': Command sent = {}".format(cmd_params[:-1])
            )  # logs without the -execute (it would confuse people to include that)
        ...
        raw_stdout = _read_fd_endswith(
            fdout, seq_ready.encode(self._encoding), self._block_size
        )

        # when it's ready, we can safely read all of stderr out, as the command is already done
        fderr = self._process.stderr.fileno()
        raw_stderr = _read_fd_endswith(
            fderr, seq_err_post.encode(self._encoding), self._block_size
        )
        ...
        cmd_stderr = raw_stderr.strip()[
            : -len(seq_err_post)
        ]  # save it in case the error below happens and output can be checked easily

        # if raw_bytes is True, the check has to become bytes rather than str
        err_status_delim = (
            SEQ_ERR_STATUS_DELIM
            if not raw_bytes
            else SEQ_ERR_STATUS_DELIM.encode(self._encoding)
        )
        ...
        if cmd_stderr[-delim_len:] != err_status_delim:
            ...
            raise ExifToolVersionError(
                f"Exiftool expected to return status on stderr, but got unexpected character: {cmd_stderr[-delim_len:]} != {err_status_delim}"
            )
        ...
        if self._logger:
            self._logger.debug(
                f"{self.__class__.__name__}.execute: IN  params = {params}"
            )
            self._logger.debug(
                f'{self.__class__.__name__}.execute: OUT stdout = "{self._last_stdout}"'
            )
            self._logger.debug(
                f'{self.__class__.__name__}.execute: OUT stderr = "{self._last_stderr}"'
            )
            self._logger.debug(
                f"{self.__class__.__name__}.execute: OUT status = {self._last_status}"
            )
    ...
    def execute_json(self, *params: Union[str, bytes]) -> List:
        ...
        if len(result) == 0:
            ...
            raise ExifToolOutputEmptyError(
                self._last_status, self._last_stdout, self._last_stderr, params
            )
        ...
        try:
            ...
            raise ExifToolJSONInvalidError(
                self._last_status, self._last_stdout, self._last_stderr, params
            ) from e
