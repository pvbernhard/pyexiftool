...
# test helpers
from tests.common_util import (
    TEST_IMAGE_DIR,
    TEST_IMAGE_JPG,
    create_random_bin_file,
    et_get_temp_dir,
)
...
class TestExifToolBytes(unittest.TestCase):
    ...
    def setUp(self):
        self.et = exiftool.ExifTool(
            ...
            config_file=TEST_IMAGE_DIR / "my_makernotes.config",
        )
    ...
    def test_read_write_binary(self):
        ...
        # get the custom tag
        written_bin_data = self.et.execute(
            "-b", "-MyMakerNotes", str(p_jpg_file), raw_bytes=True
        )
